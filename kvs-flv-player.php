...
function kvs_player_content($content) {
    if (version_compare(phpversion(), '5.4.0', '>=')) {
        $content = preg_replace_callback("'\[flv:(.*?)\]'i", function($matches) {
            return stripslashes(KVSFlvInsert($matches[1]));
        }, $content);
    } else {
...
function kvs_player_upgrade_to_pro_menu() {
    // empty
}

function kvs_player_fix_menu_item_url($url, $original_url, $_context) {
    if (preg_match('/(?:kvs_player_upgrade_to_premium)$/i', $url)) {
        remove_filter('clean_url', 'kvs_player_fix_menu_item_url', 10);
        return 'https://www.cyberseo.net/kvs-player-pro/';
    }
    return $url;
}

function kvs_player_main_menu() {
    add_menu_page('General Options', 'KVS Player', 'manage_options', 'kvs_player', 'kvs_player_general_options_menu');
    add_submenu_page('kvs_player', 'Upgrade To Premium', "<strong style=\"color: #FCB214;\">Get KVS Player Pro</strong>", "manage_options", "kvs_player_upgrade_to_premium", 'kvs_player_upgrade_to_pro_menu');
    add_filter('clean_url', 'kvs_player_fix_menu_item_url', 10, 3);
}
